...
class TfExampleDecoderTest(tf.test.TestCase):
        ...
        features=tf.train.Features(
            feature={
                ...
                'image/object/class/label':
                    dataset_util.float_list_feature(bbox_classes),
            })).SerializeToString()
  ...
  def testDecodeInstanceSegmentation(self):
    ...
    object_classes = np.random.rand(num_instances, 100).astype('float32')
    object_classes = object_classes.flatten()
  ...
  def testInstancesNotAvailableByDefault(self):
    ...
    object_classes_flattened = np.reshape(object_classes, [-1])
        ...
        features=tf.train.Features(
            feature={
                ...
                'image/object/mask':
                    dataset_util.float_list_feature(instance_masks_flattened),
                'image/object/class/label':
                    dataset_util.float_list_feature(object_classes_flattened),
            })).SerializeToString()
    example_decoder = tf_example_decoder.TfExampleDecoder()
    tensor_dict = example_decoder.decode(tf.convert_to_tensor(example))
    self.assertTrue(
